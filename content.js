...
async function waitForOnline() {
    ...
    updateOverlay(null, 'Connection Lost / Conexión perdida', 'Waiting for internet to resume...\nEsperando conexión...');
...
function showFirstKickVisitAlert() {
    ...
    extensionApi.storage.local.get(['kvd_badge_alert_shown'], (result) => {
        ...
        alert(message);
        extensionApi.storage.local.set({ kvd_badge_alert_shown: true });
    });
}
...
async function fetchVideoData(videoId) {
    try {
        await waitForOnline();
        const response = await fetchWithTimeout(`https://kick.com/api/v1/video/${videoId}`, {
            credentials: 'include'
        }, 15000);
...
function updateOverlay(progress, text = 'Downloading...', etaText = '', currentBytes = 0, currentSpeed = 0) {
    ...
    if (progress !== null) {
        ...
        try {
            extensionApi.runtime.sendMessage({ type: 'UPDATE_PROGRESS', progress: progress }).catch(() => {});
        } catch (e) { /* ignore */ }
...
function removeOverlay() {
    ...
    if (overlay) {
        // Clear badge
        try {
            extensionApi.runtime.sendMessage({ type: 'UPDATE_PROGRESS', progress: null }).catch(() => {});
        } catch (e) { /* ignore */ }
...
async function downloadSegments(streamUrl, btn, videoDurationMs, startSeconds = 0, endSeconds = -1, preOpenedHandle = null, forceMemory = false, explicitVideoId = null, downloadOptions = {}) {
    try {
        ...
        await waitForOnline();
        const response = await fetchWithTimeout(fetchUrl, { cache: 'no-store' }, 15000);
            ...
            if (variants.length > 0) {
                ...
                const variantAnalysis = await Promise.all(variants.map(async (v) => {
                    try {
                        ...
                        await waitForOnline();
                        const res = await fetchWithTimeout(vUrl, { cache: 'no-store' }, 15000);
                ...
                if (match) {
                    ...
                    while (ghostCount < MAX_GHOST_SEGMENTS && consecutiveErrors < MAX_CONSECUTIVE_ERRORS) {
                        ...
                        try {
                            ...
                            await waitForOnline();
                            const checkRes = await fetchWithTimeout(nextSegUrl, { method: 'GET' }, 15000); // Changed HEAD to GET
        ...
        const fileBaseName = `kick-vod-${explicitVideoId || getVideoId() || 'video'}`;
        let audioSourceBlob = null;
        ...
        if (writable) {
            ...
            if (isAudioOnly && handle && handle.getFile) {
                audioSourceBlob = await handle.getFile();
            }
        } else {
...
function createDownloadOptionsModal(videoId, durationMs, btn) {
    ...
    const audioConvertContainer = document.createElement('div');
    audioConvertContainer.style.cssText = 'margin: 10px 0 15px; display: flex; flex-direction: column; align-items: center; gap: 6px;';
    const audioConvertLabel = document.createElement('label');
    audioConvertLabel.style.cssText = 'font-size: 12px; color: #bbb; display: flex; align-items: center; gap: 6px;';
    const audioConvertCheckbox = document.createElement('input');
    audioConvertCheckbox.type = 'checkbox';
    ...
    (async () => {
        try {
            ...
            // Fetch Playlist
        const response = await fetchWithTimeout(masterPlaylistUrl, {}, 15000);
        const text = await response.text();
            ...
            if (variants.length > 0) {
                ...
                qualitySelect.onchange = () => {
                    ...
                    const isAudioOnlySelection = selectedVariantUrl && selectedVariantUrl.includes('#audio_only');
                    audioConvertCheckbox.disabled = !isAudioOnlySelection;
                    if (!isAudioOnlySelection) {
                        audioConvertCheckbox.checked = false;
                    }
                };
...
async function checkAutoDownloadPending() {
    if (localStorage.getItem('kvd_auto_dl_pending') === 'true') {
        ...
        try {
            ...
            await waitForOnline();
            const response = await fetchWithTimeout(`https://kick.com/api/v1/channels/${slug}`, {}, 15000);
            ...
            if (previousStreams && previousStreams.length > 0) {
                ...
                if (videoData && videoData.source) {
                    ...
                    await downloadSegments(videoData.source, dummyBtn, videoData.duration, 0, -1, null, true, videoId);
...
function findHostRejectButton() {
    const rejectKeywords = [
        'rechazar', 'reject', 'decline', 'no gracias', 'no thanks', 'cancelar host',
        'cancel host', 'rechazar host', 'stay here', 'quedarme aquí', 'quedarme'
    ];
    const hostKeywords = [
        'host', 'hosted', 'hosteo', 'hosteado', 'hosteando', 'hostear'
    ];
    ...
    for (const btn of candidates) {
        ...
        if (!label) continue;
        if (!rejectKeywords.some(keyword => label.includes(keyword))) continue;

        const dialog = btn.closest('[role="dialog"], [aria-modal="true"], [data-state="open"], .modal, .dialog');
        const contextText = (dialog ? dialog.textContent : btn.parentElement?.textContent || '').toLowerCase();

        if (hostKeywords.some(keyword => contextText.includes(keyword))) {
            return btn;
        }
    }
...
function startHostRejectionWatcher() {
    if (hostRejectionInterval) return;
    hostRejectionInterval = setInterval(() => {
        if (!isStreamerModeEnabled) return;
        const rejectBtn = findHostRejectButton();
        if (rejectBtn) {
            console.log('[KVD Host Protection] Rejecting host prompt.');
            rejectBtn.click();
        }
    }, 2000);
}

function stopHostRejectionWatcher() {
    if (hostRejectionInterval) {
        clearInterval(hostRejectionInterval);
        hostRejectionInterval = null;
    }
}
...
function injectStreamerModeUI() {
    ...
    const isDashboard = window.location.hostname === 'dashboard.kick.com';
    let target = null;
    ...
    if (isDashboard) {
        ...
        dashboardBtn.id = 'kvd-dashboard-auto-dl-btn';
        dashboardBtn.type = 'button';
        dashboardBtn.textContent = 'Auto-DL';
        dashboardBtn.title = 'Go to channel and enable Auto-DL';
        dashboardBtn.style.cssText = 'background: rgba(83, 252, 24, 0.12); color: #53fc18; border: 1px solid #53fc18; padding: 6px 12px; border-radius: 999px; font-weight: 700; cursor: pointer; font-size: 12px; box-shadow: 0 0 8px rgba(83, 252, 24, 0.2);';
        ...
        dashboardBtn.onclick = () => {
            ...
            if (confirm('⚠️ ACTIVATE & REDIRECT?\n\nEnable Auto-DL and go to channel page?\nActivar Auto-DL e ir al canal?')) {
                ...
                localStorage.setItem('kvd_channel_slug', slug);
                window.location.href = `https://kick.com/${slug}`;
            }
        };
    } else {
        // Toggle Switch (Channel Page)
...
function triggerPavleEasterEgg() {
    ...
    pavleImg.src = extensionApi.runtime.getURL('assets/pavle.png');
    pavleImg.alt = 'Pavle Toasty';
    pavleImg.className = 'kvd-pavle-toasty';
    
    // Create audio element
    const toastyAudio = document.createElement('audio');
    toastyAudio.src = extensionApi.runtime.getURL('assets/toasty.ogg');
...
function triggerMondongoEasterEgg() {
    ...
    gokuImg.src = extensionApi.runtime.getURL('assets/gokupelado.png');
    gokuImg.alt = 'Mondongo';
    gokuImg.className = 'kvd-goku-mondongo';
    
    // Create audio element
    const mondongoAudio = document.createElement('audio');
    mondongoAudio.src = extensionApi.runtime.getURL('assets/mondongo.ogg');
...
function triggerMamboEasterEgg() {
    ...
    mamboImg.src = extensionApi.runtime.getURL('assets/mambo.png');
    mamboImg.alt = 'Mambo';
    mamboImg.className = 'kvd-mambo-img';
    
    // Create audio element
    const mamboAudio = document.createElement('audio');
    mamboAudio.src = extensionApi.runtime.getURL('assets/mambo.ogg');
